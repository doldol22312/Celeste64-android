...
FOSTER_API int FosterAndroidSetupThread(void)
{
	SDL_SetMainReady();
...
void FosterStartup(FosterDesc desc)
{
	...
	// Android exposes the accelerometer as joystick 0 by default. Celeste wants real pads.
	...
	while (SDL_PollEvent(&event))
	{
		...
		else if (event.type == SDL_JOYDEVICEADDED)
		{
			...
			if (SDL_IsGameController(index) == SDL_FALSE && index >= 0 && index < FOSTER_MAX_CONTROLLERS)
			{
				...
				FosterLogInfo("Joystick connected [%i]: %s buttons=%i axes=%i vendor=%04x product=%04x", index, name, button_count, axis_count, vendor, product);
				fstate.desc.onControllerConnect(index, name, button_count, axis_count, 0, vendor, product, version);
			}
		}
		...
		else if (event.type == SDL_CONTROLLERDEVICEADDED)
		{
			...
			if (index >= 0 && index < FOSTER_MAX_CONTROLLERS)
			{
				...
				FosterLogInfo("Gamecontroller connected [%i]: %s vendor=%04x product=%04x", index, name, vendor, product);
